...
function RestView() {
  const {id} = useParams(); //to  get particular id this hook is used
  console.log("id",id);
  ...
  console.log("selected reastauatnt");
  ...
  return (
    ...
    <Row className='mt-5 mb-5'>
        <Col md={1}> </Col>
        <Col md={3} lg={3}>
        <img src={selectedRestaurant[0]?.photograph} alt="" width={"100%"} className='rounded'/>
        </Col>
        <Col md={7} lg={7}>
        <hr />
        <h4 className='text-center'>
       <span className='text-warning me-2'>Restaurant</span>Details
        </h4>
        <hr />
        <ListGroup>
      <ListGroup.Item><h4 className='text'>{selectedRestaurant[0]?.name}</h4></ListGroup.Item>
      ...
      <ListGroup.Item className='text-center p-3'><button className='btn btn-success' onClick={handleShow}>Operating Hours</button>
    ...
    <Modal show={show} onHide={handleClose} >
        <Modal.Header closeButton  style={{ backgroundColor: '#198754' }}>
      ...
      <ListGroup.Item>Sunday: <span className='text-primary'> {selectedRestaurant[0].operating_hours.Sunday} </span></ListGroup.Item>
        ...
        <Modal.Footer  style={{ backgroundColor: '#198754' }}>
